...
export const meetingsRouter = createTRPCRouter({
    ...
    generateToken:protectedProcedure.mutation(async ({ctx})=>{
        ...
        const expirationTime= Math.floor(Date.now()/1000) + 60*60 // 1 hours
        const isSueAt= Math.floor(Date.now()/1000)-60 // current time in seconds
    ...
    create: premiumProcedure('meetings')
        ...
        .mutation(async ({input, ctx})=>{
            ...
            .values({
                    ...
                    agent_Id: input.agentId//this may be removed
                })
            .returning()
            ...
            await call.create({
             ...
             const [existingAgent]= await db
                .select()
                .from(agent)
                .where(eq(agent.id, createdMeeting.agent_Id))
    ...
    getOne: protectedProcedure
        ...
        .query(async ({input,ctx})=>{
            ...
            .from(meetings)
            .innerJoin(agent, eq(meetings.agent_Id, agent.id))
            .where(
                and(
                    eq(meetings.id, input.id),
                    eq(meetings.user , ctx.auth.user.id)
